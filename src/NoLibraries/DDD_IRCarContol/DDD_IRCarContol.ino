...
const int turn_speed_matrix[] = {5, 10, 15, 20, 25, 30, 35, 40, 45, 50};
  ...
  if (irrecv.decode(&results)) { // have we received an IR signal?
    ...
    switch (results.value) {
      ...
      case IR_CODE_POUND:
        ...
        break;
      case IR_CODE_REPEAT:
        if (curState == CAR_STATE_FORWARD || curState == CAR_STATE_BACK || curState == CAR_STATE_RIGHT || curState == CAR_STATE_LEFT) {
          loop_timer = micros();
        }
...
void statefullCarMotion() {
  switch (curState) {
    ...
    case CAR_STATE_RIGHT:
      car_Right(255 - (255 * turn_speed_percent / 100));
      break;
    case CAR_STATE_LEFT:
      car_Left(255 - (255 * turn_speed_percent / 100));
      break;
    case CAR_STATE_STOP:
      car_Stop();
      break;
