...
void readPWMIn(int *rx_pwm_signal) {
  ...
  for (int i = 0; i < RX_CHANNELS; ++i) {
    if (rx_pwm_signal[i] <= PWM_MIN)
      rx_pwm_signal[i] = PWM_MIN;
    if (rx_pwm_signal[i] >= PWM_MAX)
      rx_pwm_signal[i] = PWM_MAX;
  }
}
...
void interruptCalcPWM(int i, int port_pin) {
  ...
  else if (rx_last_PWM_state[i] &&
           !(port_pin & pwm_input_pin[i])) { // Did Input change from HIGH(true) to LOW(false) AND Is the input pin LOW?
    rx_last_PWM_state[i] = false;            // Remember current input state (false/LOW)
    rx_signal[i] =
        rx_timer_current - rx_timer[i]; // Channel microseconds value: current time - timer value (range should be 1000us - 2000us)
  }
}
...
void printRCPWMValues() {
  Serial.print("Roll:");
  if (rx_signal[RC_CH_IDX_ROLL] - (PWM_MID - PWM_DEADBAND) < 0)
    Serial.print("<<<"); // RCPWM - 1480 if Deadband is 20
  else if (rx_signal[RC_CH_IDX_ROLL] - (PWM_MID + PWM_DEADBAND) > 0)
  ...
  Serial.print("  Pitch:");
  if (rx_signal[RC_CH_IDX_PITCH] - (PWM_MID - PWM_DEADBAND) < 0)
    Serial.print("^^^"); // RCPWM - 1480 if Deadband is 20
  else if (rx_signal[RC_CH_IDX_PITCH] - (PWM_MID + PWM_DEADBAND) > 0)
  ...
  if (rx_signal[RC_CH_IDX_THROTTLE] - (PWM_MID - PWM_DEADBAND) < 0)
    Serial.print("vvv"); // RCPWM - 1480 if Deadband is 20
  else if (rx_signal[RC_CH_IDX_THROTTLE] - (PWM_MID + PWM_DEADBAND) > 0)
    Serial.print("^^^"); // RCPWM - 1520 if Deadband is 20
  else
    Serial.print("-+-");
  ...
  Serial.print("  Yaw:");
  if (rx_signal[RC_CH_IDX_YAW] - (PWM_MID - PWM_DEADBAND) < 0)
    Serial.print("<<<"); // RCPWM - 1480 if Deadband is 20
  else if (rx_signal[RC_CH_IDX_YAW] - (PWM_MID + PWM_DEADBAND) > 0)
    Serial.print(">>>"); // RCPWM - 1520 if Deadband is 20
  else
    Serial.print("-+-");
